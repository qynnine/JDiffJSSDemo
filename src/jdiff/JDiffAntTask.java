...
/**
 * An Ant task to produce a simple JDiff report. More complex reports still need
 * parameters that are controlled by the Ant Javadoc task.
 */
public class JDiffAntTask {
	...
	public void execute() throws BuildException {
		...
		if (jdiffHome == null || jdiffHome.compareTo("") == 0
			...
			throw new BuildException(
					"Error: invalid JDIFF_HOME property. Set it in the build file to the directory where jdiff is installed");
		}
		project.log(" JDiff home: " + jdiffHome, Project.MSG_INFO);
		//SK - all used packages need to be added here separately to buildpath
		jdiffClassPath = jdiffHome + DIR_SEP + "jdiff.jar"
				+ System.getProperty("path.separator") + jdiffHome + DIR_SEP
				+ "xerces.jar" + System.getProperty("path.separator")
				+ jdiffHome + DIR_SEP + "org.apache.bcel.jar"
				+ System.getProperty("path.separator") + jdiffHome + DIR_SEP
				+ "callGraph.jar" + System.getProperty("path.separator") + jdiffHome + DIR_SEP + "KeyWordExtracting.jar"
				+ System.getProperty("path.separator") + jdiffHome + DIR_SEP + "tools.jar" 
				+ System.getProperty("path.separator") + jdiffHome + DIR_SEP + "lemmatizer.jar";
		 ...
		 * if (getVerbose()) { project.log("Older version: " +
		 * oldProject.getName(), Project.MSG_INFO);
		 * project.log("Included directories for older version:",
		 * Project.MSG_INFO); DirectoryScanner ds =
		 * oldProject.getDirset().getDirectoryScanner(project); String[] files =
		 * ds.getIncludedDirectories(); for (int i = 0; i < files.length; i++) {
		 * project.log(" " + files[i], Project.MSG_INFO); } ds = null;
		 * 
		 * project.log("Newer version: " + newProject.getName(),
		 * Project.MSG_INFO);
		 * project.log("Included directories for newer version:",
		 * Project.MSG_INFO); ds =
		 * newProject.getDirset().getDirectoryScanner(project); files =
		 * ds.getIncludedDirectories(); for (int i = 0; i < files.length; i++) {
	...
	/**
	 * Convenient method to create a Javadoc task, configure it and run it to
	 * generate the XML representation of a project's source files.
	 * 
	 * @param proj
	 *            The current Project
	 */
	protected void generateXML(ProjectInfo proj) {
		...
		//SK - visibility attribute was missing, added jd.setPrivate(true)
		jd.setPrivate(true);
	...
	/**
	 * Convenient method to create a Javadoc task, configure it and run it to
	 * compare the XML representations of two instances of a project's source
	 * files, and generate an HTML report summarizing the differences.
	 * 
	 * @param oldapiname
	 *            The name of the older version of the project
	 * @param newapiname
	 *            The name of the newer version of the project
	 */
	protected void compareXML(String oldapiname, String newapiname) {
		...
		if (getStats()) {
			...
			copyFile(jdiffHome + DIR_SEP + "black.gif", getDestdir().toString()
					+ DIR_SEP + "black.gif");
			copyFile(jdiffHome + DIR_SEP + "background.gif", getDestdir()
					.toString() + DIR_SEP + "background.gif");
		}
	...
	/**
	 * Generate the Javadoc for the project. If you want to generate the Javadoc
	 * report for the project with different parameters from the simple ones
	 * used here, then use the Javadoc Ant task directly, and set the javadoc
	 * attribute to the "old" or "new" element.
	 * 
	 * @param proj
	 *            The current Project
	 */
	...
	protected void addSourcePaths(Javadoc jd, ProjectInfo proj) {
		...
		for (int i = 0; i < numDirSets; i++) {
			...
			jd.setSourcepath(new Path(project, dirSet.getDir(project)
					.toString()));
		}
	}

	/**
	 * Return the comma-separated list of packages. The list is generated from
	 * Ant DirSet tasks, and includes all directories in a hierarchy, e.g. com,
	 * com/acme. com/acme/foo. Duplicates are ignored.
	 */
	...
	protected void copyFile(String src, String dst) {
		...
		try {
			File reportSubdir = new File(getDestdir().toString() + DIR_SEP
					+ "changes");
			...
			project.log("Warning: unable to copy " + src.toString() + " to "
					+ dst.toString(), Project.MSG_WARN);
	...
	/**
	 * The JDiff Ant task does not inherit from an Ant task, such as the Javadoc
	 * task, though this is usually how most Tasks are written. This is because
	 * JDiff needs to run Javadoc three times (twice for generating XML, once
	 * for generating HTML). The Javadoc task has not easy way to reset its list
	 * of packages, so we needed to be able to crate new Javadoc task objects.
	 * 
	 * Note: Don't confuse this class with the ProjectInfo used by JDiff. This
	 * Project class is from Ant.
	 */
	...
	/**
	 * JDIFF_HOME must be set as a property in the Ant build file. It should be
	 * set to the root JDiff directory, ie. the one where jdiff.jar is found.
	 */
	...
	/**
	 * The destination directory for the generated report. The default is
	 * "./jdiff_report".
	 */
	...
	/**
	 * Increases the JDiff Ant task logging verbosity if set with "yes", "on" or
	 * true". Default has to be false. To increase verbosity of Javadoc, start
	 * Ant with -v or -verbose.
	 */
	...
	/**
	 * Used to store the child element named "old", which is under the JDiff
	 * task XML element.
	 */
	...
	/**
	 * Used to store the child element named "new", which is under the JDiff
	 * task XML element.
	 */
	...
	/**
	 * This class handles the information about a project, whether it is the
	 * older or newer version.
	 * 
	 * Note: Don't confuse this class with the Project used by Ant. This
	 * ProjectInfo class is from local to this task.
	 */
	public static class ProjectInfo {
		/**
		 * The name of the project. This is used (without spaces) as the base of
		 * the name of the file which contains the XML representing the project.
		 */
		private String name;
		...
		/**
		 * The location of the Javadoc HTML for this project. Default value is
		 * "generate", which will cause the Javadoc to be generated in a
		 * subdirectory named "name" in the task's destdir directory.
		 */
		...
		/**
		 * These are the directories which contain the packages which make up
		 * the project. Filesets are not supported by JDiff.
		 */
