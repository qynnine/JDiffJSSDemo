...
import KeyWordExtracting.Change;
import KeyWordExtracting.KeyWordExtractor;
import KeyWordExtracting.Settings;
import KeyWordExtracting.WeighedTerms;
import KeyWordExtracting.ChangeObject;
...
public class HTMLReportGenerator {
	// SK - additional global Attributes
	public static JCallGraph callGraphV1;

	/*private double classWeighting = Settings.classWeighting;
	private double classWeightingPrivate = Settings.classWeightingPrivate;
	private double methodWeighting = Settings.methodWeighting;
	private double methodWeightingPrivate = Settings.methodWeightingPrivate;
	private double pckgWeighting = Settings.pckgWeighting;
	private double cstrWeighting = Settings.cstrWeighting;
	private double fieldWeighting = Settings.fieldWeighting;
	private double fieldWeightingPrivate = Settings.fieldWeightingPrivate;
	private double docWeighting = Settings.documentationWeighting;
	private double callHierarchyWeighting = Settings.callHierarchyWeighting;*/

	/** KeyWordExtractors to store changes (added, removed and changed) */
	public static KeyWordExtractor extractorAdded = new KeyWordExtracting.KeyWordExtractor();
	public static KeyWordExtractor extractorRemoved = new KeyWordExtractor();
	public static KeyWordExtractor extractorChanged = new KeyWordExtractor();

	/**
	 * Hashtables to store fields and Methods which were added and removed. Used
	 * for determing global changes in multiple classes.
	 */
	public Hashtable<String, Integer> fieldsAdded = new Hashtable<String, Integer>();
	public Hashtable<String, Integer> fieldsRemoved = new Hashtable<String, Integer>();
	public Hashtable<String, Integer> methodsAdded = new Hashtable<String, Integer>();
	public Hashtable<String, Integer> methodsRemoved = new Hashtable<String, Integer>();

	/** Default constructor. */
	public HTMLReportGenerator() {
		callGraphV1 = new JCallGraph(Settings.JarVersion1Path);

		// System.out.println(callGraphV1.isCalledByGraphMap.toString());
	}
	...
	/**
	 * The Comments object for freshly regenerated comments. This is populated
	 * during the generation of the report, and should be like existingComments_
	 * but with unused comments marked as such, so that they can be commented
	 * out in XML when the new comments are written out to the comments file.
	 */
	private Comments newComments_ = null;

	/**
	 * Accessor method for the freshly generated Comments object. The list of
	 * comments is sorted before the object is returned.
	 */
	...
	public void generate(APIComparator comp, Comments existingComments) {
		...
		System.out.println("JDiff: generating HTML report into the file '"
				+ fullReportFileName + reportFileExt
				+ "' and the subdirectory '" + fullReportFileName + "'");
		...
		if (!opdir.mkdir() && !opdir.exists()) {
			System.out.println("Error: could not create the subdirectory '"
					+ fullReportFileName + "'");
			System.exit(3);
		}
		...
		try {
			...
			if (windowTitle == null)
				writeHTMLTitle("API Differences between " + oldAPIName
						+ " and " + newAPIName);
			else
			...
			writeNavigationBar(reportFileName + "-summary", null, null, null,
					0, true, apiDiff.packagesRemoved.size() != 0,
			...
			if (docTitle == null) {
				...
				writeText("</center>");
				writeText("<center>");
				writeText("<H2>Between " + oldAPIName + " and " + newAPIName
						+ "</H2>");
				writeText("</center>");
			} else {
				writeText(docTitle);
			...
			System.out.println("IO Error while attempting to create "
					+ changesSummaryName);
		...
		String tlf = fullReportFileName + JDiff.DIR_SEP + "jdiff_topleftframe"
				+ reportFileExt;
		...
		h.emitAllBottomLeftFiles(packagesIndexName, classesIndexName,
				constructorsIndexName, methodsIndexName, fieldsIndexName,
				allDiffsIndexName, apiDiff);

		if (doStats) {
			// The file for the statistical report.
			String sf = fullReportFileName + JDiff.DIR_SEP + "jdiff_statistics"
					+ reportFileExt;
			HTMLStatistics stats = new HTMLStatistics(this);
			stats.emitStatistics(sf, apiDiff);
		}
		// Adaptions SK for KeyWordExtracting
		// if Callhierarchy-Weighting is enables (>0) the callhierarchy is added
		// to the changes
		if (Settings.callHierarchyWeighting != 0) {
			extractorChanged.addCallHierarchy(1);
			extractorRemoved.addCallHierarchy(1);
			extractorAdded.addCallHierarchy(2);
		}
		// compare added to removed changes to detect and filter out renamings
		extractorAdded.checkRenamings(extractorRemoved);
		// remove standard words from the KeyWordExtractors (Words defined in
		// KeyWordExtracting.Settings.java)
		extractorAdded.removeStdWords();
		extractorRemoved.removeStdWords();
		extractorChanged.removeStdWords();
		// Sort changes in KeyWordExtractors according to their weighting
		extractorAdded.sort();
		extractorRemoved.sort();
		extractorChanged.sort();
		// Write changes to console
		System.out.println("Added: ");
		extractorAdded.print();
		System.out.println("");
		System.out.println("Removed: ");
		extractorRemoved.print();
		System.out.println("");
		System.out.println("Changed: ");
		extractorChanged.print();
		// write changes to HTML Report grouped by added/removed/changed
		writeExtractedChangesToHTML();
		// write changes to HTML Report grouped by class
		writeExtractedChangesToHTMLGroupedByClass2();
	}

	/**
	 * Write Changes To HTML File,Grouped By Class
	 * 
	 * Writes all the collected changes to the HTML File
	 * "HTMLReportGroupedByClass.html"
	 * 
	 * @author Simon Kaeser, University Of Zurich
	 */
	public void writeExtractedChangesToHTMLGroupedByClass() {
/*		// Hashtable used for grouping keywords by class
		Hashtable<String, Vector<Change>> classGrouping = new Hashtable<String, Vector<Change>>();
		// change number
		int changeNr = 0;
		try {
			// generates new html file
			FileWriter htmlFile = new FileWriter(
					"HTMLReportGroupedByClass.html");

			htmlFile.write("<html><head><title>Changes Grouped By Class</title></head><body>");

			htmlFile.append("<p><h2>Global Changes:</h2></p>");
			// add global changes (methods, fields), which appear in more than
			// one class
			htmlFile.append("<h3>Added Fields in more than one class</h3>");
			Enumeration<String> fieldsAddedEnum = fieldsAdded.keys();
			while (fieldsAddedEnum.hasMoreElements()) {
				String fieldName = fieldsAddedEnum.nextElement();
				if (fieldsAdded.get(fieldName) > 1) {
					htmlFile.append("Field <b>" + fieldName
							+ "</b> has been addded to "
							+ fieldsAdded.get(fieldName) + " classes.<br />");
				} else
					fieldsAdded.remove(fieldName);
			}
			htmlFile.append("<h3>Removed Fields in more than one class</h3>");
			Enumeration<String> fieldsRemovedEnum = fieldsRemoved.keys();
			while (fieldsRemovedEnum.hasMoreElements()) {
				String fieldName = fieldsRemovedEnum.nextElement();
				if (fieldsRemoved.get(fieldName) > 1) {
					htmlFile.append("Field <b>" + fieldName
							+ "</b> has been removed from "
							+ fieldsRemoved.get(fieldName) + " classes.<br />");
				} else
					fieldsRemoved.remove(fieldName);
			}
			htmlFile.append("<h3>Removed Methods in more than one class</h3>");
			Enumeration<String> methodsRemovedEnum = methodsRemoved.keys();
			while (methodsRemovedEnum.hasMoreElements()) {
				String methodName = methodsRemovedEnum.nextElement();
				if (methodsRemoved.get(methodName) > 1) {
					htmlFile.append("Method<b> " + methodName
							+ "</b> has been removed from "
							+ methodsRemoved.get(methodName)
							+ " classes.<br />");
				} else
					methodsRemoved.remove(methodName);
			}
			htmlFile.append("<h3>Added Methods from more than one class</h3>");
			Enumeration<String> methodsAddedEnum = methodsAdded.keys();
			while (methodsAddedEnum.hasMoreElements()) {
				String methodName = methodsAddedEnum.nextElement();
				if (methodsAdded.get(methodName) > 1) {
					htmlFile.append("Method<b> " + methodName
							+ "</b> has been added to "
							+ methodsAdded.get(methodName) + " classes.<br />");
				} else
					methodsAdded.remove(methodName);
			}

			Vector<KeyWordExtractor> extractors = new Vector<KeyWordExtractor>();
			extractors.add(extractorAdded);
			extractors.add(extractorRemoved);
			extractors.add(extractorChanged);
			Enumeration extractorsEnum = extractors.elements();
			htmlFile.append("<p><h2>Changes:</h2></p>");
			// group all changes from the 3 extractors according to their class
			// belonging
			while (extractorsEnum.hasMoreElements()) {
				KeyWordExtractor kwExtr = (KeyWordExtractor) extractorsEnum
						.nextElement();

				Enumeration<Integer> keys1 = kwExtr.ht.keys();
				while (keys1.hasMoreElements()) {
					int i = keys1.nextElement();

					if (classGrouping.isEmpty()) {
						Vector<Change> tempVector = new Vector<Change>();
						tempVector.add(kwExtr.ht.get(i));
						classGrouping.put(kwExtr.ht.get(i).getClassName(),
								tempVector);
					} else {
						if (classGrouping.containsKey(kwExtr.ht.get(i)
								.getClassName())) {
							if (!classGrouping.containsValue(i)) {
								classGrouping.get(
										kwExtr.ht.get(i).getClassName()).add(
										kwExtr.ht.get(i));
							}
						} else {
							Vector<Change> tempVector = new Vector<Change>();
			...
			// write all the changes grouped by class in the html file
			Enumeration<String> classGroupingKeys = classGrouping.keys();
			while (classGroupingKeys.hasMoreElements()) {
				String classGroupingKey = classGroupingKeys.nextElement();
				Vector<Change> extractorObjects = new Vector<Change>();
				extractorObjects = classGrouping.get(classGroupingKey);
				Hashtable<String, Double> noDuplicates = new Hashtable<String, Double>();
				htmlFile.append("<p><h4>Change " + changeNr + ": "
						+ classGroupingKey + "</h4></p>");
				changeNr++;
				for (int i = extractorObjects.size() - 1; i >= 0; i--) {
					String name = extractorObjects.get(i).getName();
					// if condition for filtering out global changes
					// if(!(fieldsAdded.containsKey(name)||fieldsRemoved.containsKey(name)||methodsAdded.containsKey(name)||methodsRemoved.containsKey(name))){
					htmlFile.append("<p><i><font size=\"-1\">"
							+ extractorObjects.get(i).getExtractionType()
							+ ": " + extractorObjects.get(i).getFullName()
							+ "</i><br />");
					for (int j = 0; j < extractorObjects.get(i)
							.getDifferenceVector().size(); j++) {
						String word = extractorObjects.get(i)
								.getDifferenceVector().get(j).getWord();
						Double weighting = extractorObjects.get(i)
								.getDifferenceVector().get(j).getWeighting();
						if (noDuplicates.containsKey(word)) {
							if (noDuplicates.get(word) < weighting) {
								noDuplicates.put(word, weighting);
							}
						} else {
							noDuplicates.put(word, weighting);
						}
						htmlFile.append(extractorObjects.get(i)
								.getDifferenceVector().get(j).getWord()
								+ "("
								+ extractorObjects.get(i).getDifferenceVector()
										.get(j).getWeighting() + ") ");
					}
					htmlFile.append("</font></p>");
					// bracket for if (filtering out global changes
					// }
				}
				// add all keywords sorted according to their keyword at the end
				// of each change class group, without duplicates
				htmlFile.append("<font color=\"#0000FF\">");
				double highestWeighting;
				String highestWeightingPosition = "";

				while (!noDuplicates.isEmpty()) {
					highestWeighting = -1;
					Enumeration<String> keys = noDuplicates.keys();
					while (keys.hasMoreElements()) {
						String i = keys.nextElement();
						if (noDuplicates.get(i) > highestWeighting) {
							highestWeighting = noDuplicates.get(i);
							highestWeightingPosition = i;
						}
					}
					htmlFile.append(highestWeightingPosition + "("
							+ highestWeighting + ") ");
					noDuplicates.remove(highestWeightingPosition);
				}
	...
	/**
	 * Sorts a Hashtable according to the weighting of their keywords, sorting
	 * beginning with the highest weighting.
	 * 
	 * @param ht
	 *            Hashtable which need to be sorted.
	 * @return htNew sorted Hashtable<String,Double>
	 * @author Simon Kaeser, University of Zurich
	 */
	public Hashtable<String, Double> sortHashtable(Hashtable<String, Double> ht) {
		Hashtable<String, Double> htNew = new Hashtable<String, Double>();
		double highestWeighting;
		String highestWeightingPosition = "";
		while (!ht.isEmpty()) {
			highestWeighting = -1;
			...
			while (keys.hasMoreElements()) {
				String i = keys.nextElement();
				if (ht.get(i) > highestWeighting) {
					highestWeighting = ht.get(i);
					highestWeightingPosition = i;
				}
			}
			htNew.put(highestWeightingPosition, highestWeighting);
			ht.remove(highestWeightingPosition);
		}
		return htNew;
	}

	/**
	 * Write extracted changes to HTML File "HTMLReport.html". Keywords are
	 * grouped by added, removed, changed
	 * 
	 * @author Simon Kaeser, University of Zurich @
	 */
	public void writeExtractedChangesToHTML() {
		try {
			// create html file "HTMLReport.html"
			FileWriter htmlFile = new FileWriter("HTMLReport.html");

			htmlFile.write("<html><head><title>Changes</title></head><body>");
			...
			while (extractorsEnum.hasMoreElements()) {
				KeyWordExtractor kwExtr = (KeyWordExtractor) extractorsEnum
						.nextElement();
				if (kwExtr.equals(extractorAdded))
					htmlFile.append("<p><h2>Added:</h2></p>");
				else if (kwExtr.equals(extractorRemoved))
					htmlFile.append("<p><h2>Removed:</h2></p>");
				else if (kwExtr.equals(extractorChanged))
					htmlFile.append("<p><h2>Changed:</h2></p>");

				// add all the changes from the added, removed, changed
				// KeyWordExtractor to the html file
				Enumeration<Integer> keys = kwExtr.ht.keys();
				while (keys.hasMoreElements()) {
					int i = keys.nextElement();
					htmlFile.append("<p><h4>Change " + i + ": "
							+ kwExtr.ht.get(i).getExtractionType() + ": "
							+ kwExtr.ht.get(i).getFullName() + "</h4></p>");
					for (int j = 0; j < kwExtr.ht.get(i).differenceVector
							.size(); j++) {
						htmlFile.append(kwExtr.ht.get(i).getDifferenceVector()
								.get(j).getWord()
								+ "("
								+ kwExtr.ht.get(i).getDifferenceVector().get(j)
										.getWeighting() + ") ");
					}
				}
			}
	 ...
	 * removed, and the changed packages section has links which takes you to a
	 * section for each package. This pattern continues for classes and
	 * constructors, methods and fields.
	 */
	public void writeReport(APIDiff apiDiff) {

		// Report packages which were removed in the new API
		System.out.println("Package removed size " + apiDiff.packagesRemoved.size());// trace
		if (apiDiff.packagesRemoved.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				// SK - writes removed packagename into keywordextractor
				Change change = new Change(pkgName, "Removed Package", pkgName);
				change.setName(pkgName);
				change.add(Settings.pckgWeighting, pkgName, pkgName);
				extractorRemoved.addChange(change);
				// END SK
		...
		if (apiDiff.packagesAdded.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				// SK - writes added packagename into keywordextractor
				Change change = new Change(pkgName, "Added Package", pkgName);
				change.setName(pkgName);
				change.add(Settings.pckgWeighting, pkgName, pkgName);
				extractorAdded.addChange(change);
				// END SK
			}
			writeTableEnd();
		}
		...
		if (apiDiff.packagesChanged.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				// writes changed packagename into keywordextractor, not
				// necessary anymore, results in duplicate entries in list
				// (information that package is changed is already in the
				// specific class adding/removing information
				// System.out.println("Test HTMLReportGenerator: Changed Packages: "
				// + pkgName);
				if (Settings.considerPkgChanges) {
					Change change = new Change(pkgName);
					change.add(Settings.pckgWeighting, pkgName, pkgName);
					extractorChanged.addChange(change);
				}
			}
			writeTableEnd();
			...
			PackageDiff[] pkgDiffs = new PackageDiff[apiDiff.packagesChanged
					.size()];
			pkgDiffs = (PackageDiff[]) apiDiff.packagesChanged
					.toArray(pkgDiffs);
	...
	public void reportChangedPackage(PackageDiff[] pkgDiffs, int pkgIndex) {
		...
		try {
			...
			// with the same name.
			localReportFileName = reportFileName + JDiff.DIR_SEP + "pkg_"
					+ pkgName + reportFileExt;
			if (outputDir != null)
				localReportFileName = outputDir + JDiff.DIR_SEP
						+ localReportFileName;
		...
		String linkedPkgName = "<A HREF=\"" + pkgRef
				+ ".html\" target=\"_top\"><tt>" + pkgName + "</tt></A>";
		...
		writeSectionHeader("Package " + linkedPkgName, pkgName, prevPkgRef,
				nextPkgRef, null, 1, pkgDiff.classesRemoved.size() != 0,
		...
		if (reportDocChanges && pkgDiff.documentationChange_ != null) {
			...
			if (oldPkgRef != null)
				pkgDiff.documentationChange_ += "<A HREF=\""
						+ oldPkgRef
		...
		if (pkgDiff.classesRemoved.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				if (tracing)
					System.out.println("Class/Interface " + className
							+ " was removed.");
				writeClassTableEntry(pkgName, className, 0,
						classAPI.isInterface_, classAPI.doc_, false);

				// SK - writes removed classes into KeyWordExtractor
				double localWeighting = Settings.classWeighting;
				// if modifier is private use classWeightingPrivate
				if (classAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.classWeightingPrivate;
				}
				// if weighting is zero change will not be considered
				if (localWeighting != 0) {
					Change change = new Change(pkgName + "." + className,
							"Removed Class", pkgName + "." + className);
					change.setName(className);
					change.add(localWeighting, pkgName + "." + className,
							className);
					
					//EBC -  Added the documentation of the class to the terms
					if(classAPI.doc_!=null)
						change.addDoc(Settings.docWeighting, pkgName + "." + className, classAPI.doc_);
					//end EBC
					extractorRemoved.addChange(change);
				}
			}
		...
		if (pkgDiff.classesAdded.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				if (tracing)
					System.out.println("Class/Interface " + className
							+ " was added.");
				writeClassTableEntry(pkgName, className, 1,
						classAPI.isInterface_, classAPI.doc_, false);

				// SK -writes added classes into keywordextractor
				double localWeighting = Settings.classWeighting;
				// if modifier is private use classWeightingPrivate
				if (classAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.classWeightingPrivate;
		...
		if (pkgDiff.classesChanged.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				if (tracing)
					System.out.println("Package " + pkgDiff.name_
							+ ", class/Interface " + className
							+ " was changed.");
				writeClassTableEntry(pkgName, className, 2,
						classDiff.isInterface_, null, false);

				// SK - writes changed classes into keywordextractor
				double localWeighting = Settings.classWeighting;
				// if modifier is private use classWeightingPrivate
				if (classDiff.getModifiersVisibility_().equalsIgnoreCase(
						"private")) {
					System.out.println("chanegd class " + classDiff.name_);
					localWeighting = Settings.classWeightingPrivate;
				}
				// if weighting is zero change will not be considered
				if (localWeighting != 0) {
					Change change = new Change(pkgName + "." + className,
							"Changed Class", pkgName + "." + className);
					change.setName(className);
					change.add(localWeighting, pkgName + "." + className,
							className);
					
					//EBC add documentation of the new class to the change
					if(classDiff.doc_!=null)
					{
						change.addDoc(Settings.docWeighting, pkgName + "." + className, classDiff.doc_);
					}
					//end EBC
			...
			ClassDiff[] classDiffs = new ClassDiff[pkgDiff.classesChanged
					.size()];
			classDiffs = (ClassDiff[]) pkgDiff.classesChanged
					.toArray(classDiffs);
	 ...
	 */
	public void reportChangedClass(String pkgName, ClassDiff[] classDiffs,
			int classIndex) {
		...
		String localReportFileName = null;
		try {
			localReportFileName = reportFileName + JDiff.DIR_SEP + pkgName
					+ "." + className + reportFileExt;
			if (outputDir != null)
				localReportFileName = outputDir + JDiff.DIR_SEP
						+ localReportFileName;
			FileOutputStream fos = new FileOutputStream(localReportFileName);
		...
		String linkedClassName = "<A HREF=\"" + classRef
				+ ".html\" target=\"_top\"><tt>" + className + "</tt></A>";
		String lcn = pkgName + "." + linkedClassName;
		// Links to the previous and next classes
		String prevClassRef = null;
		if (classIndex != 0) {
			prevClassRef = pkgName + "." + classDiffs[classIndex - 1].name_
					+ reportFileExt;
		}
		// Create the HTML link to the next package
		String nextClassRef = null;
		if (classIndex < classDiffs.length - 1) {
			nextClassRef = pkgName + "." + classDiffs[classIndex + 1].name_
					+ reportFileExt;
		}
		...
		writeSectionHeader(lcn, pkgName, prevClassRef, nextClassRef, className,
				2, hasCtors, hasMethods, hasFields);

		if (classDiff.inheritanceChange_ != null)
			writeText("<p><font size=\"+1\">" + classDiff.inheritanceChange_
					+ "</font>");
		...
		if (classDiff.modifiersChange_ != null)
			writeText("<p><font size=\"+1\">" + classDiff.modifiersChange_
					+ "</font>");
	...
	public void reportAllCtors(String pkgName, ClassDiff classDiff) {
		...
		if (classDiff.ctorsRemoved.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				writeCtorTableEntry(pkgName, className, ctorType, 0,
						ctorAPI.doc_, false);
				// SK: write removed ctors into keywordextractor
				// if weighting is zero change will not be considered
				if (Settings.cstrWeighting != 0) {
					Change change = new Change(pkgName + "." + id,
							"Removed Constructor", pkgName + "." + className);
					change.setName(id);
					change.add(Settings.cstrWeighting, pkgName + "." + id, className);
					extractorRemoved.addChange(change);
				}
			}
			writeTableEnd();
		...
		if (classDiff.ctorsAdded.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				writeCtorTableEntry(pkgName, className, ctorType, 1,
						ctorAPI.doc_, false);
				// SK - write added ctors into keywordextractor
				// if weighting is zero change will not be considered
				if (Settings.cstrWeighting != 0) {
					Change change = new Change(pkgName + "." + id,
							"Added Constructor", pkgName + "." + className);
					change.setName(id);
					change.add(Settings.cstrWeighting, pkgName + "." + id, className);
					extractorAdded.addChange(change);
				}
			...
			while (iter.hasNext()) {
				...
				if (tracing)
					System.out.println("Constructor for " + className
							+ " was changed from " + memberDiff.oldType_
							+ " to " + memberDiff.newType_);
				writeCtorChangedTableEntry(pkgName, className, memberDiff);

				// SK - write changed ctors into keywordextractor
				// if weighting is zero change will not be considered
				if (Settings.considerCstrChanges) {
					if (Settings.cstrWeighting != 0) {
						Change change = new Change(pkgName + "." + className,
								"Changed Constructor", pkgName + "."
										+ className);
						change.setName(className);
						change.add(Settings.cstrWeighting, pkgName + "." + className,
								className);
						extractorChanged.addChange(change);
					}
				}
	...
	public void reportAllMethods(String pkgName, ClassDiff classDiff) {
		...
		if (classDiff.methodsRemoved.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				String methodName = methodAPI.name_ + "("
						+ methodAPI.getSignature() + ")";
				if (tracing)
					System.out
							.println("Method " + methodName + " was removed.");
				writeMethodTableEntry(pkgName, className, methodAPI, 0,
						methodAPI.doc_, false);
				
				// SK - write removed methods into keywordextractor
				double localWeighting = Settings.methodWeighting;
				// if visibility is private use methodWEightingPrivate
				if (methodAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.methodWeightingPrivate;
				}
				// if weighting is zero change will not be considered also if it
				// is a setter/getter method
				if (localWeighting != 0 
						//&& !methodAPI.name_.startsWith("get")
						//&& !methodAPI.name_.startsWith("set")
						) {
					Change change = new Change(pkgName + "." + className + "."
							+ methodAPI.name_, "Removed Method", pkgName + "."
							+ className);
					change.setName(methodAPI.name_);
					change.add(localWeighting, pkgName + "." + className + "."
							+ methodName, className + "." + methodAPI.name_);
					
					//EBC -  Added the documentation of the method to the terms
					if(methodAPI.doc_!=null)
						change.addDoc(Settings.docWeighting, pkgName + "." + className + "."
								+ methodName, methodAPI.doc_);
					extractorRemoved.addChange(change);
					// add global methodsRemoved
					if (methodsRemoved.containsKey(methodAPI.name_)) {
						int value = methodsRemoved.get(methodAPI.name_);
						methodsRemoved.put(methodAPI.name_, value + 1);
					} else {
						methodsRemoved.put(methodAPI.name_, 1);
					}
				}
			}
			writeTableEnd();
		...
		if (classDiff.methodsAdded.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				String methodName = methodAPI.name_ + "("
						+ methodAPI.getSignature() + ")";
				if (tracing)
					System.out.println("Method " + methodName + " was added.");
				writeMethodTableEntry(pkgName, className, methodAPI, 1,
						methodAPI.doc_, false);
				// SK - write added methods into keywordextractor
				double localWeighting = Settings.methodWeighting;
				if (methodAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.methodWeightingPrivate;
				}
				// if weighting is zero change will not be considered, also when
				// it is a setter/getter method
				if (localWeighting != 0 
						//&& !methodAPI.name_.startsWith("get")
						//&& !methodAPI.name_.startsWith("set")
						) {
					Change change = new Change(pkgName + "." + className + "."
							+ methodAPI.name_, "Added Method", pkgName + "."
							+ className);
					change.setName(methodAPI.name_);
					change.add(localWeighting, pkgName + "." + className + "."
							+ methodName, className + "." + methodAPI.name_);
					...
					if(methodAPI.doc_!=null)
						change.addDoc(Settings.docWeighting, pkgName + "." + className + "."
								+ methodName, methodAPI.doc_);
					extractorAdded.addChange(change);
					// add global methodsAdding
					if (methodsAdded.containsKey(methodAPI.name_)) {
						int value = methodsAdded.get(methodAPI.name_);
						methodsAdded.put(methodAPI.name_, value + 1);
					} else {
						methodsAdded.put(methodAPI.name_, 1);
					}
				}
			}
			writeTableEnd();
		...
		if (classDiff.methodsChanged.size() != 0) {
			...
			while (iter.hasNext()) {
				MemberDiff memberDiff = (MemberDiff) (iter.next());
				if (tracing)
					System.out.println("Method " + memberDiff.name_
							+ " was changed.");
				writeMethodChangedTableEntry(pkgName, className, memberDiff);
				
				// SK - writes methodnames into keywordextractor
				if (Settings.considerMethodChanges) {
					double localWeighting = Settings.methodWeighting;
					// if visibility is private use methodWeightingPrivate
					if (memberDiff.modifiersVisibility_
							.equalsIgnoreCase("private")) {
						localWeighting = Settings.methodWeightingPrivate;
					}
					// if weighting is zero change will not be considered, also
					// when
					// it is a getter/setter method
					if (Settings.considerMethodChanges && localWeighting != 0) {
							//&& !memberDiff.name_.startsWith("get")
							//&& !memberDiff.name_.startsWith("set")
						Change change = new Change(pkgName + "." + className
								+ "." + memberDiff.name_, "Changed Method",
								pkgName + "." + className);
						change.setName(memberDiff.name_);
						change.add(localWeighting, pkgName + "." + className
								+ "." + memberDiff.name_, className + "."
								+ memberDiff.name_);
						extractorChanged.addChange(change);
					}
				}
	...
	public void reportAllFields(String pkgName, ClassDiff classDiff) {
		...
		if (classDiff.fieldsRemoved.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				writeFieldTableEntry(pkgName, className, fieldAPI, 0,
						fieldAPI.doc_, false);

				// SK - add removed fielsd to keywordextractor
				double localWeighting = Settings.fieldWeighting;
				// if visibility is private use methodWeightingPrivate
				if (fieldAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.fieldWeightingPrivate;
				}
				// if weighting is zero change will not be considered
				if (localWeighting != 0) {
					Change change = new Change(pkgName + "." + className + "."
							+ fieldName, "Removed Field", pkgName + "."
							+ className);
					change.setName(fieldName);
					change.add(localWeighting, pkgName + "." + className + "."
							+ fieldName, fieldName);
					change.add(Settings.subElementWeighting, pkgName + "."
							+ className + "." + fieldName, className);
					extractorRemoved.addChange(change);
					// add global FieldsRemoving
					if (fieldsRemoved.containsKey(fieldName)) {
						int value = fieldsRemoved.get(fieldName);
						fieldsRemoved.put(fieldName, value + 1);
					} else {
						fieldsRemoved.put(fieldName, 1);
					}
				}
			}
			writeTableEnd();
		...
		if (classDiff.fieldsAdded.size() != 0) {
			...
			while (iter.hasNext()) {
				...
				String fieldName = fieldAPI.name_;
				if (tracing)
					System.out.println("Field " + fieldName + " was added.");
				writeFieldTableEntry(pkgName, className, fieldAPI, 1,
						fieldAPI.doc_, false);

				// SK - add added field to keywordextractor
				double localWeighting = Settings.fieldWeighting;
				if (fieldAPI.modifiers_.visibility.equalsIgnoreCase("private")) {
					localWeighting = Settings.fieldWeightingPrivate;
				}
				...
				if (localWeighting != 0) {
					...
					change.add(Settings.subElementWeighting, pkgName + "."
							+ className + "." + fieldName, className);
					extractorAdded.addChange(change);
					// add global FieldsAdding
					if (fieldsAdded.containsKey(fieldName)) {
						int value = fieldsAdded.get(fieldName);
						fieldsAdded.put(fieldName, value + 1);
					} else {
						fieldsAdded.put(fieldName, 1);
					}
				}
			}
			writeTableEnd();
		...
		if (classDiff.fieldsChanged.size() != 0) {
			...
			while (iter.hasNext()) {
				MemberDiff memberDiff = (MemberDiff) (iter.next());
				if (tracing)
					System.out.println("Field " + pkgName + "." + className
							+ "." + memberDiff.name_ + " was changed from "
							+ memberDiff.oldType_ + " to "
							+ memberDiff.newType_);
				writeFieldChangedTableEntry(pkgName, className, memberDiff);
				// SK - add changed fields to keywordextractor
				if (Settings.considerAttributChanges) {
					double localWeighting = Settings.fieldWeighting;
					if (memberDiff.modifiersVisibility_
							.equalsIgnoreCase("private")) {
						localWeighting = Settings.fieldWeightingPrivate;
					...
					if (localWeighting != 0) {
						Change change = new Change(pkgName + "." + className
								+ "." + memberDiff.name_, "Changed Field",
								pkgName + "." + className);
						change.setName(memberDiff.name_);
						change.add(localWeighting, pkgName + "." + className
								+ "." + memberDiff.name_, memberDiff.name_);
						extractorChanged.addChange(change);
					}
				}
			}
	...
	/**
	 * Write the start of the HTML header, together with the current date and
	 * time in an HTML comment.
	 */
	...
	public void writeStartHTMLHeader(boolean addDate) {
		writeText("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Frameset//EN\"\""
				+ RootDocToXML.baseURI + "/TR/REC-html40/frameset.dtd\">");
		writeText("<HTML>");
		writeText("<HEAD>");
		...
		writeText("<meta name=\"description\" content=\""
				+ JDiff.jDiffDescription + "\">");
	...
	public void writeStyleSheetRef(boolean inSameDir) {
		if (inSameDir)
			...
			writeText("<LINK REL=\"stylesheet\" TYPE=\"text/css\" HREF=\""
					+ "../" + "stylesheet-jdiff.css\" TITLE=\"Style\">");
		// This doesn't work in non-windows browsers, so have to change the
		// stylesheet
	 ...
	 * 
	 * @param title
	 *            Title of the header. Contains any links necessary.
	 * @param packageName
	 *            The name of the current package, with no slashes or links in
	 *            it. May be null
	 * @param prevElemLink
	 *            An HTML link to the previous element (a package or class). May
	 *            be null.
	 * @param nextElemLink
	 *            An HTML link to the next element (a package or class). May be
	 *            null.
	 * @param className
	 *            The name of the current class, with no slashes or links in it.
	 *            May be null.
	 * @param level
	 *            0 = overview, 1 = package, 2 = class/interface
	 */
	public void writeSectionHeader(String title, String packageName,
			String prevElemLink, String nextElemLink, String className,
			int level, boolean hasRemovals, boolean hasAdditions,
			boolean hasChanges) {
		writeNavigationBar(packageName, prevElemLink, nextElemLink, className,
				level, true, hasRemovals, hasAdditions, hasChanges);
		if (level != 0) {
	 ...
	 *            0 = overview, 1 = package, 2 = class/interface
	 */
	public void writeSectionFooter(String packageName, String prevElemLink,
			String nextElemLink, String className, int level) {
	 ...
	 *            0 = overview, 1 = package, 2 = class/interface
	 */
	public void writeNavigationBar(String pkgName, String prevElemLink,
			String nextElemLink, String className, int level,
			boolean upperNavigationBar, boolean hasRemovals,
			boolean hasAdditions, boolean hasChanges) {
		reportFile.println("<!-- Start of nav bar -->");
		reportFile
				.println("<TABLE summary=\"Navigation bar\" BORDER=\"0\" WIDTH=\"100%\" CELLPADDING=\"1\" CELLSPACING=\"0\">");
		reportFile.println("  <TR>");
		reportFile
				.println("    <TD COLSPAN=2 BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\">");
		reportFile
				.println("    <TABLE summary=\"Navigation bar\" BORDER=\"0\" CELLPADDING=\"0\" CELLSPACING=\"3\">");
		...
		if (atOverview) {
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\""
							+ newDocPrefix
							+ "index.html\" target=\"_top\"><FONT CLASS=\"NavBarFont1\"><B><tt>"
							+ apiDiff.newAPIName_
							+ "</tt></B></FONT></A>&nbsp;</TD>");
		} else if (atPackage) {
			String pkgRef = pkgName;
			...
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\""
							+ pkgRef
							+ ".html\" target=\"_top\"><FONT CLASS=\"NavBarFont1\"><B><tt>"
							+ apiDiff.newAPIName_
							+ "</tt></B></FONT></A>&nbsp;</TD>");
		} else if (atClass) {
			String classRef = pkgName + "." + className;
		...
		if (atOverview) {
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1Rev\"> &nbsp;<FONT CLASS=\"NavBarFont1Rev\"><B>Overview</B></FONT>&nbsp;</TD>");
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> &nbsp;<FONT CLASS=\"NavBarFont1\">Package</FONT>&nbsp;</TD>");
			reportFile
					.println("      <TD BGCOLOR=\"#FFFFFF\" CLASS=\"NavBarCell1\"> &nbsp;<FONT CLASS=\"NavBarFont1\">Class</FONT>&nbsp;</TD>");
		}
		...
		if (atPackage) {
			...
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1Rev\"> &nbsp;<FONT CLASS=\"NavBarFont1Rev\"><B>Package</B></FONT>&nbsp;</TD>");
			reportFile
					.println("      <TD BGCOLOR=\"#FFFFFF\" CLASS=\"NavBarCell1\"> &nbsp;<FONT CLASS=\"NavBarFont1\">Class</FONT>&nbsp;</TD>");
		}
		if (atClass) {
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\""
							...
							+ "\"><FONT CLASS=\"NavBarFont1\"><B>Overview</B></FONT></A>&nbsp;</TD>");
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\"pkg_"
							+ pkgName
							+ reportFileExt
		...
		if (!Diff.noDocDiffs) {
			if (atPackage) {
				String id = (String) Diff.firstDiffOutput.get(pkgName
						+ "!package");
				if (id != null)
					reportFile
							.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\""
									...
									+ "#"
									+ id
									+ "\"><FONT CLASS=\"NavBarFont1\"><B>Text Changes</B></FONT></A>&nbsp;</TD>");
				else
					reportFile
							.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <FONT CLASS=\"NavBarFont1\">Text Changes</FONT>&nbsp;</TD>");
			} else if (atClass) {
				String id = (String) Diff.firstDiffOutput.get(pkgName + "."
						+ className + "!class");
				if (id != null)
		...
		if (doStats) {
			reportFile
					.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\"jdiff_statistics"
							+ reportFileExt
							+ "\"><FONT CLASS=\"NavBarFont1\"><B>Statistics</B></FONT></A>&nbsp;</TD>");
		}

		// Always have a link to the JDiff help file
		reportFile
				.println("      <TD BGCOLOR=\"#EEEEFF\" CLASS=\"NavBarCell1\"> <A HREF=\"jdiff_help"
						+ reportFileExt
		...
		if (upperNavigationBar) {
			reportFile
					.println("  <TD ALIGN=\"right\" VALIGN=\"top\" ROWSPAN=3><EM><b>Generated by<br><a href=\""
							+ JDiff.jDiffLocation
							+ "\" class=\"staysblack\" target=\"_top\">JDiff</a></b></EM></TD>");
		} else {
			reportFile
					.println("  <TD ALIGN=\"right\" VALIGN=\"top\" ROWSPAN=3></TD>");
		}
		reportFile.println("</TR>");
		...
		reportFile.println("  <TD BGCOLOR=\"" + bgcolor
		...
		if (atPackage || atClass) {
			...
			if (prevElemLink == null) {
				...
				reportFile.println("&nbsp;<A HREF=\"" + prevElemLink
						+ "\"><B>PREV " + elemName + "</B></A>");
			}
			if (nextElemLink == null) {
				reportFile.println("&nbsp;<B>NEXT " + elemName + "</B>&nbsp;");
			} else {
				reportFile.println("&nbsp;<A HREF=\"" + nextElemLink
						+ "\"><B>NEXT " + elemName + "</B></A>");
			}
			reportFile
					.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
		...
		reportFile.println("  <A HREF=\"" + "../" + reportFileName
				+ reportFileExt
		...
		if (className == null) {
			...
			reportFile.println("  &nbsp;<A HREF=\"" + pkgName + "." + className
					+ reportFileExt
					+ "\" TARGET=\"_top\"><B>NO FRAMES</B></A></FONT></TD>");
		}
		...
		if (atClass) {
			...
			if (hasCtors || hasMethods || hasFields) {
				reportFile
						.println("  <TD BGCOLOR=\""
								+ bgcolor
								+ "\" CLASS=\"NavBarCell3\"><FONT SIZE=\"-2\"> DETAIL: &nbsp;");
				if (hasCtors) {
					reportFile
							.println("<a href=\"#constructors\">CONSTRUCTORS</a>&nbsp;|&nbsp;");
				} else {
					reportFile.println("CONSTRUCTORS&nbsp;|&nbsp;");
				}
				if (hasMethods) {
					reportFile
							.println("<a href=\"#methods\">METHODS</a>&nbsp;|&nbsp;");
				} else {
				...
				reportFile
						.println("<TD BGCOLOR=\"0xFFFFFF\" CLASS=\"NavBarCell3\"></TD>");
			}
		} else {
			...
			if (hasRemovals || hasAdditions || hasChanges) {
				...
				if (hasRemovals) {
					reportFile
							.println("<a href=\"#Removed\">REMOVED</a>&nbsp;|&nbsp;");
				} else {
					reportFile.println("REMOVED&nbsp;|&nbsp;");
				}
				if (hasAdditions) {
					reportFile
							.println("<a href=\"#Added\">ADDED</a>&nbsp;|&nbsp;");
				} else {
	...
	public void writeTableStart(String title, int colSpan) {
		...
		reportFile.println("<a NAME=\"" + namedAnchor + "\"></a>"); // Named
																	// anchor
		reportFile
		...
		reportFile.print("  <TD VALIGN=\"TOP\" COLSPAN=" + colSpan
				+ "><FONT SIZE=\"+1\">");
	...
	public String makeTwoRows(String name) {
		...
		if (idx == -1)
			return name;
		int len = name.length();
		String res = name.substring(0, idx + 1) + "<br>"
				+ name.substring(idx + 1, len);
		return res;
	}

	/**
	 * Write a table entry for a package, with support for links to Javadoc for
	 * removed packages.
	 * 
	 * linkType: 0 - no link by default, 1 = link to Javadoc HTML file, 2 = link
	 * to JDiff file
	 */
	public void writePackageTableEntry(String pkgName, int linkType,
		...
		if (!useOld) {
			...
			reportFile.println("  <A NAME=\"" + pkgName + "\"></A>"); // Named
																		// anchor
		}
		...
		if (linkType == 0) {
			...
			if (useOld)
				...
				pkgRef = newDocPrefix + pkgRef + "/package-summary";
			reportFile.println("  <nobr><A HREF=\"" + pkgRef
					+ ".html\" target=\"_top\"><tt>" + shownPkgName
					+ "</tt></A></nobr>");
		} else if (linkType == 2) {
			reportFile.println("  <nobr><A HREF=\"pkg_" + pkgName
					+ reportFileExt + "\">" + shownPkgName + "</A></nobr>");
		}
		if (!useOld) {
			reportFile.println("  </TD>");
	...
	public void writeClassTableEntry(String pkgName, String className,
			int linkType, boolean isInterface, String possibleComment,
			boolean useOld) {
		if (!useOld) {
			reportFile.println("<TR BGCOLOR=\"" + bgcolor
					+ "\" CLASS=\"TableRowColor\">");
			reportFile.println("  <TD VALIGN=\"TOP\" WIDTH=\"25%\">");
			reportFile.println("  <A NAME=\"" + className + "\"></A>"); // Named
																		// anchor
		}
		...
		if (linkType == 0) {
			if (oldDocPrefix == null) {
				...
				writeClassTableEntry(pkgName, className, 1, isInterface,
			...
			reportFile.print("  <nobr><A HREF=\"" + classRef
					+ ".html\" target=\"_top\"><tt>");
			if (isInterface)
	...
	public void writeCtorTableEntry(String pkgName, String className,
			String type, int linkType, String possibleComment, boolean useOld) {
		String fqName = pkgName + "." + className;
		String shownClassName = makeTwoRows(className);
		String lt = "removed";
		if (linkType == 1)
		...
		if (!useOld) {
			reportFile.println("<TR BGCOLOR=\"" + bgcolor
					+ "\" CLASS=\"TableRowColor\">");
			reportFile.println("  <TD VALIGN=\"TOP\" WIDTH=\"25%\">");
			reportFile.println("  <A NAME=\"" + commentID + "\"></A>"); // Named
																		// anchor
		}
		...
		if (linkType == 0) {
			if (oldDocPrefix == null) {
				...
				writeCtorTableEntry(pkgName, className, type, 1,
						possibleComment, true);
			}
		} else if (linkType == 1) {
			// Link to HTML file for the package
			...
			reportFile.print("  <nobr><A HREF=\"" + memberRef + ".html#"
					+ className + "(" + type + ")\" target=\"_top\"><tt>"
					+ shownClassName + "</tt></A>");
			emitTypeWithParens(shortType);
			reportFile.println("</nobr>");
		}
	...
	public void writeCtorChangedTableEntry(String pkgName, String className,
		...
		reportFile.print("  <nobr><A HREF=\"" + memberRef + ".html#"
				+ className + "(" + newType + ")\" target=\"_top\"><tt>");
		reportFile.print(shownClassName);
		...
		if (reportDocChanges && memberDiff.documentationChange_ != null) {
			...
			if (oldDocPrefix != null)
				memberDiff.documentationChange_ += "<A HREF=\"" + oldMemberRef
						+ ".html#" + className + "(" + oldType
	...
	public void writeMethodTableEntry(String pkgName, String className,
			MethodAPI methodAPI, int linkType, String possibleComment,
			boolean useOld) {
		String fqName = pkgName + "." + className;
		...
		if (linkType == 1)
			lt = "added";
		String commentID = fqName + "." + methodName + "_" + lt + "("
				+ signature + ")";
		if (!useOld) {
			reportFile.println("<TR BGCOLOR=\"" + bgcolor
					+ "\" CLASS=\"TableRowColor\">");
		...
		if (linkType == 0) {
			...
			reportFile.print("&nbsp;<A HREF=\"" + memberRef + ".html#"
					+ methodName + "(" + signature + ")\" target=\"_top\"><tt>"
					+ methodName + "</tt></A>");
			emitTypeWithParens(shortSignature);
			reportFile.println("</nobr>");
		}
	...
	public void writeMethodChangedTableEntry(String pkgName, String className,
		...
		String commentID = fqName + "." + memberName + "_changed("
				+ newSignature + ")";
		reportFile.println("<TR BGCOLOR=\"" + bgcolor
				+ "\" CLASS=\"TableRowColor\">");
		...
		if (className.indexOf('.') == -1
				...
				&& memberDiff.modifiersChange_
						.indexOf("but is now inherited from") != -1) {
		...
		reportFile.print("&nbsp;<A HREF=\"" + memberRef + ".html#" + memberName
				+ "(" + newSignature + ")\" target=\"_top\"><tt>");
		...
		if (memberDiff.modifiersChange_ != null) {
			int parentIdx = memberDiff.modifiersChange_
					.indexOf("now inherited from");
			if (parentIdx != -1) {
	...
	public void writeFieldTableEntry(String pkgName, String className,
			FieldAPI fieldAPI, int linkType, String possibleComment,
			boolean useOld) {
		String fqName = pkgName + "." + className;
		...
		if (linkType == 0) {
			...
			reportFile.print("  <nobr>");
			emitType(shortFieldType);
			reportFile.println("&nbsp;<A HREF=\"" + memberRef + ".html#"
					+ fieldName + "\" target=\"_top\"><tt>" + fieldName
					+ "</tt></A></nobr>");
		}
		if (!useOld) {
			reportFile.println("  </TD>");
	...
	public void emitChanges(MemberDiff memberDiff, int memberType) {
		...
		if (memberType != 2
				...
				&& memberDiff.oldExceptions_
						.compareTo(memberDiff.newExceptions_) != 0) {
			if (hasContent)
				reportFile.print(" ");
	...
	/**
	 * Emit a string which is an exception by surrounding it with &lt;code&gt;
	 * tags. If there is a space in the type, e.g. &quot;String, File&quot;,
	 * then surround it with parentheses too. Do not add &lt;code&gt; tags or
	 ...
	 * parameters. Suggestions for where a browser should break the text are
	 * provided with &lt;br> and &ltnobr> tags.
	 */
	...
	public static String simpleName(String fqNames) {
		...
		for (String fqName : fqNamesList) {
			...
			if (hasContent)
				res += ", ";
			hasContent = true;
			// Look for text within '<' and '>' in case this is a invocation of
			// a generic
			...
			if (firstBracket != -1 && lastBracket != -1) {
				genericParameter = simpleName(fqName.substring(
						firstBracket + 1, lastBracket));
	...
	/**
	 * Find any existing comment and emit it. Add the new comment to the list of
	 * new comments. The first instance of the string "@first" in a hand-written
	 * comment will be replaced by the first sentence from the associated doc
	 * block, if such exists. Also replace @link by an HTML link.
	 * 
	 * @param commentID
	 *            The identifier for this comment.
	 * @param possibleComment
	 *            A possible comment from another source.
	 * @param linkType
	...
	public void emitComment(String commentID, String possibleComment,
		...
		if (!noCommentsOnChanges && possibleComment == null) {
			possibleComment = (String) Comments.allPossibleComments
					.get(commentID);
		}
		...
		if (comment.compareTo(Comments.placeHolderText) == 0) {
			if (possibleComment != null
				...
				reportFile.println("  <TD VALIGN=\"TOP\">" + possibleComment
						+ "</TD>");
			else
				reportFile.println("  <TD>&nbsp;</TD>");
		} else {
			...
			if (idx == -1) {
				reportFile.println("  <TD VALIGN=\"TOP\">"
						+ Comments.convertAtLinks(comment, "", null, null)
						+ "</TD>");
			} else {
				reportFile.print("  <TD VALIGN=\"TOP\">"
						+ comment.substring(0, idx));
				if (possibleComment != null
						&& possibleComment.indexOf("InsertOtherCommentsHere") == -1)
	...
	/**
	 * The name of the file to which the top-level HTML file is written, and
	 * also the name of the subdirectory where most of the HTML appears, and
	 * also a prefix for the names of some of the files in that subdirectory.
	 */
	...
	/**
	 * The object which represents the top of the tree of differences between
	 * two APIs. It is only used indirectly when emitting a navigation bar.
	 */
	static APIDiff apiDiff = null;

	/**
	 * If set, then do not suggest comments for removals from the first sentence
	 * of the doc block of the old API.
	 */
	...
	/**
	 * If set, then do not suggest comments for changes from the first sentence
	 * of the doc block of the new API.
	 */
	public static boolean noCommentsOnChanges = false;

	/**
	 * If set, then report changes in documentation (Javadoc comments) between
	 * the old and the new API. The default is that this is not set.
	 */
	 ...
	 * &quot;API Differences Between &lt;name of old API&gt; and &lt;name of new
	 * API&gt;&quot;. It can be set by using the -doctitle option.
	 */
	public static String docTitle = null;

	/**
	 * The browser window title for the report. By default, this is &quot;API
	 * Differences Between &lt;name of old API&gt; and &lt;name of new
	 * API&gt;&quot;. It can be set by using the -windowtitle option.
	 */
	...
	public void writeExtractedChangesToHTMLGroupedByClass2() {
		// Hashtable used for grouping keywords by class
		Hashtable<String, Vector<Change>> classGrouping = new Hashtable<String, Vector<Change>>();
		// change number
		int changeNr = 0;
		try {
			...
			FileWriter keywordsFile2 = new FileWriter("./keywords/filesNames");
			BufferedWriter out2 = new BufferedWriter(keywordsFile2);
			// generates new html file
			FileWriter htmlFile = new FileWriter(
					"HTMLReportGroupedByClass.html");
			...
			while (extractorsEnum.hasMoreElements()) {
				...
				while (keys1.hasMoreElements()) {
					...
					if (classGrouping.isEmpty()) {
						...
						//TODO Eya : check why the getclassname returns null, is it because some classes are anonymous?
						if(kwExtr.ht.get(i).getClassName()==null){
							System.out.println("no classname found for change: " + kwExtr.ht.get(i).getFullName());
							System.out.println("extraction type: " +  kwExtr.ht.get(i).getExtractionType());
						}
							
						else if (classGrouping.containsKey(kwExtr.ht.get(i)
								.getClassName())) {
							if (!classGrouping.containsValue(i)) {
								classGrouping.get(
										kwExtr.ht.get(i).getClassName()).add(
			...
			while (classGroupingKeys.hasMoreElements()) {
				...
				Vector<Change> extractorObjects = new Vector<Change>();
				extractorObjects = classGrouping.get(classGroupingKey);
				Hashtable<String, Double> classSummaryListNoDuplicates = new Hashtable<String, Double>();
				Vector<ChangeObject> classSummaryListFull = new Vector<ChangeObject>();
				htmlFile.append("<p><h4>Change " + changeNr + ": "
						+ classGroupingKey + "</h4></p>");
				changeNr++;
				for (int i = extractorObjects.size() - 1; i >= 0; i--) {
					...
					for (int j = 0; j < extractorObjects.get(i)
						...
						if (Settings.noDuplicates) {
							if (classSummaryListNoDuplicates.containsKey(word)) {
								if (classSummaryListNoDuplicates.get(word) < weighting) {
									classSummaryListNoDuplicates.put(word, weighting);
								}
							} else {
								classSummaryListNoDuplicates.put(word, weighting);
							}
						} else {
							ChangeObject tempChangeObject = new ChangeObject(weighting, word, word);
							classSummaryListFull.add(tempChangeObject);
						}
						htmlFile.append(extractorObjects.get(i)
								.getDifferenceVector().get(j).getWord()
								+ "("
								+ extractorObjects.get(i).getDifferenceVector()
										.get(j).getWeighting() + ") ");
					//	out.write(extractorObjects.get(i).getDifferenceVector().get(j).getWord()
								//+ "("+ extractorObjects.get(i).getDifferenceVector().get(j).getWeighting() + ") ");
					}
					htmlFile.append("</font></p>");
					// bracket for if (filtering out global changes
					// }
				...
				// add all keywords sorted according to their keyword at the end
				// of each change class group, without duplicates
				htmlFile.append("<p><font color=\"#0000FF\">");
				double highestWeighting;
				String highestWeightingPosition = "";
				int change = changeNr-1;
				//FileWriter keywordsFile = new FileWriter("./keywords/kW"+change+"-"+classGroupingKey.substring(classGroupingKey.lastIndexOf(".")+1));
				FileWriter keywordsFile = new FileWriter("./keywords/kW"+change+"-"+classGroupingKey);
				BufferedWriter out = new BufferedWriter(keywordsFile);
				//out2.append(change+"	"+classGroupingKey.substring(classGroupingKey.lastIndexOf(".")+1));// to get the list of all the files
				out2.append(change+"	"+classGroupingKey);// to get the list of all the files
				out2.newLine();
				
				if(Settings.noDuplicates){
				while (!classSummaryListNoDuplicates.isEmpty()) {
					highestWeighting = -1;
					Enumeration<String> keys = classSummaryListNoDuplicates.keys();
					while (keys.hasMoreElements()) {
						String i = keys.nextElement();
						if (classSummaryListNoDuplicates.get(i) > highestWeighting) {
							highestWeighting = classSummaryListNoDuplicates.get(i);
							highestWeightingPosition = i;
						}
					}
					htmlFile.append(highestWeightingPosition + "("
							+ highestWeighting + ") ");
					out.write(highestWeightingPosition + "("
							+ highestWeighting + ") ");
					classSummaryListNoDuplicates.remove(highestWeightingPosition);
				}
				}
				else{
					int highestWeightingPositionFullList=-1;
					int listSize=classSummaryListFull.size();
					for(int i=0;i<listSize;i++){
						highestWeighting = -1;
						for (int j=0;j<classSummaryListFull.size();j++){
							if (classSummaryListFull.get(j).getWeighting() > highestWeighting) {
								highestWeighting = classSummaryListFull.get(j).getWeighting();
								highestWeightingPositionFullList = j;
							}
						}
						
						htmlFile.append("</font></p>");
						htmlFile.append("<font color=\"#FF00FF\">");
						//print weighting in summary according to Settings.printWeighting
						if(Settings.printWeighting){
							htmlFile.append(classSummaryListFull.get(highestWeightingPositionFullList).getWord() + "("
									+ highestWeighting + ") ");
							out.write(classSummaryListFull.get(highestWeightingPositionFullList).getWord() + "("
									+ highestWeighting + ") ");
							classSummaryListFull.remove(highestWeightingPositionFullList);							
						}
						else{
							htmlFile.append(classSummaryListFull.get(highestWeightingPositionFullList).getWord() + " ");
							out.write(classSummaryListFull.get(highestWeightingPositionFullList).getWord() + " ");
