...
/**
 * This class contains method to compare two API objects. The differences are
 * stored in an APIDiff object.
 * 
...
public class APIComparator {
	...
	/**
	 * Top-level object representing the differences between two APIs. It is
	 * this object which is used to generate the report later on.
	 */
	...
	/** SK - hashtable to save what added/removed packages contain */
	public static Hashtable<String, Vector<String>> packageContent = new Hashtable<String, Vector<String>>();
	/** SK - hashtable to save what added/removed classes contain */
	public static Hashtable<String, Vector<String>> classContent = new Hashtable<String, Vector<String>>();
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (existsNew != -1) {
					...
					differs += 2.0 * comparePackages(oldPkg,
							(PackageAPI) (newAPI.packages_.get(existsNew)));
				} else {
					if (trace)
					...
					// SK - output classes of newly removed pckgs
					// System.out.println("New Pckg removed: Classes: " +
					// oldPkg.classes_.toString());
					// SK - create vector<string> where classes and methods are
					// stored
					Vector<String> classAndMethodVector = new Vector<String>();

					// EBC TODO: add the documentation of the new classes and methods
					for (int i = 0; i < oldPkg.classes_.size(); i++) {
						classAndMethodVector.add(((ClassAPI) oldPkg.classes_
								.get(i)).getName_());
//						ClassAPI tempClass = (ClassAPI) oldPkg.classes_.get(i);
//						for (int j = 0; j < (tempClass.methods_.size()); j++) {
//							classAndMethodVector
//									.add(((MethodAPI) tempClass.methods_.get(j))
//											.getName_());
//						}
					}
					// add classes (and methods) to corresponding removed package
					packageContent.put(oldPkg.name_, classAndMethodVector);
					// END SK
				...
				// be compared.
				differs += 2.0 * comparePackages(oldPkg,
						(PackageAPI) (newAPI.packages_.get(idx)));
			}
		} // while (iter.hasNext())
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (existsOld != -1) {
					...
					if (trace)
						System.out.println("Package " + newPkg.name_
								+ " was added");
					apiDiff.packagesAdded.add(newPkg);
					differs += 1.0;

					// SK - output classes of newly added pckgs
					// System.out.println("New Pckg added: Classes: " +
					// newPkg.classes_.toString());
					// create new vector<string> for storing of corresponding
					// classes/methods
					Vector<String> classAndMethodVector = new Vector<String>();

					for (int i = 0; i < newPkg.classes_.size(); i++) {
						classAndMethodVector.add(((ClassAPI) newPkg.classes_
								.get(i)).getName_());
//						ClassAPI tempClass = (ClassAPI) newPkg.classes_.get(i);
//						for (int j = 0; j < (tempClass.methods_.size()); j++) {
//							classAndMethodVector
//									.add(((MethodAPI) tempClass.methods_.get(j))
//											.getName_());
//						}
					...
					// add classes (and methods) to corresponding added package
					packageContent.put(newPkg.name_, classAndMethodVector);
					// END SK
		...
		//
		// The definition ensures that if all classes are removed and all new
		// classes
		// added, the change will be 100%.
		// Evaluation of the visibility of elements has already been done when
		// the
		// XML was written out.
		...
		if (trace)
			System.out.println("Top level changes: " + differs + "/"
					+ denom.intValue());
		...
		if (approxPercentage == 0)
			System.out.println(" Approximately " + percentage
					+ "% difference between the APIs");
		else
			System.out.println(" Approximately " + approxPercentage
					+ "% difference between the APIs");
	...
	public double comparePackages(PackageAPI oldPkg, PackageAPI newPkg) {
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (existsNew != -1) {
					...
					// but there has been some or other change.
					differs += 2.0 * compareClasses(oldClass,
							(ClassAPI) (newPkg.classes_.get(existsNew)),
							pkgDiff);
				} else {
					if (trace) {
						System.out.println("  Class " + oldClass.name_
								+ " was removed");
					}

					// SK - output methods of newly removed classes
					// System.out.println("New Class added: Methods: "
					// +oldClass.methods_.toString());
					String word = "";
					Vector<String> tempVector = new Vector<String>();

					for (int i = 0; i < oldClass.methods_.size(); i++) {
						tempVector.add(((MethodAPI) oldClass.methods_.get(i))
								.getName_());
						
						//EBC we add the documentation of the method the terms
						if(((MethodAPI) oldClass.methods_.get(i)).doc_!=null){
							tempVector.add(((MethodAPI) oldClass.methods_.get(i))
									.doc_);
						}
						
					}
					for (int i = 0; i < oldClass.fields_.size(); i++) {
						tempVector.add(((FieldAPI) oldClass.fields_.get(i))
								.getName_());
					}

					classContent.put(newPkg.name_ + "." + oldClass.name_,
							tempVector);
					// END SK
				...
				differs += 2.0 * compareClasses(oldClass,
						(ClassAPI) (newPkg.classes_.get(idx)), pkgDiff);
			}
		} // while (iter.hasNext())
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (existsOld != -1) {
					...
					if (trace) {
						System.out.println("  Class " + newClass.name_
								+ " was added");
					}

					//SK - output methods of newly added classes
					Vector<String> tempVector = new Vector<String>();
					for (int i = 0; i < newClass.methods_.size(); i++) {
						tempVector.add(((MethodAPI) newClass.methods_.get(i))
								.getName_());
						//EBC we add the documentation of the method the terms
						if(((MethodAPI) newClass.methods_.get(i)).doc_!=null){
							tempVector.add(((MethodAPI) newClass.methods_.get(i)).doc_);
						}
								
					}
					for (int i = 0; i < newClass.fields_.size(); i++) {
						tempVector.add(((FieldAPI) newClass.fields_.get(i))
								.getName_());
					}
					classContent.put(newPkg.name_ + "." + newClass.name_,
							tempVector);
					//END SK
		...
		if (docChanged(oldPkg.doc_, newPkg.doc_)) {
			String link = "<a href=\"pkg_" + oldPkg.name_
					+ HTMLReportGenerator.reportFileExt
					+ "\" class=\"hiddenlink\">";
			String id = oldPkg.name_ + "!package";
			String title = link + "Package <b>" + oldPkg.name_ + "</b></a>";
			pkgDiff.documentationChange_ = Diff.saveDocDiffs(pkgDiff.name_,
					null, oldPkg.doc_, newPkg.doc_, id, title);
			differsFlag = true;
		}
		...
		if (denom.intValue() == 0) {
			System.out.println("Warning: no classes found in the package "
					+ oldPkg.name_);
			return 0.0;
		}
		if (trace)
			System.out.println("Package " + pkgDiff.name_
					+ " had a difference of " + differs + "/"
					+ denom.intValue());
	 ...
	 */
	public double compareClasses(ClassAPI oldClass, ClassAPI newClass,
			PackageDiff pkgDiff) {
		if (trace)
		...
		//EBC add new class documentation to the change
		classDiff.setNewDocumentation(newClass.doc_);
		//end EBC
		
		classDiff.isInterface_ = newClass.isInterface_; // Used in the report
		classDiff.setModifiersVisibility_(oldClass.modifiers_.visibility);
		...
		if (oldClass.isInterface_ != newClass.isInterface_) {
			...
			//SK - add possibility to enable/disable comparing of class headers
		...
		if (modifiersChange != null) {
			...
			if (modifiersChange
					.indexOf("Change from deprecated to undeprecated") != -1) {
				System.out
						.println("JDiff: warning: change from deprecated to undeprecated for class "
								+ pkgDiff.name_ + "." + newClass.name_);
		...
		boolean differsCtors = compareAllCtors(oldClass, newClass, classDiff);
		boolean differsMethods = compareAllMethods(oldClass, newClass,
				classDiff);
		boolean differsFields = compareAllFields(oldClass, newClass, classDiff);
		...
		if (trace)
			System.out.println("  Class " + classDiff.name_
					+ " had a difference of " + differs + "/"
					+ denom.intValue());
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (oldSize == 1 && oldSize == newSize) {
					...
					ConstructorAPI newCtor = (ConstructorAPI) (newClass.ctors_
							.get(0));
					...
					if (docChanged(oldCtor.doc_, newCtor.doc_)) {
						...
						if (type.compareTo("void") == 0)
							type = "";
						String fqName = pkgDiff.name_ + "." + classDiff.name_;
						String link1 = "<a href=\"" + fqName
								+ HTMLReportGenerator.reportFileExt
								+ "\" class=\"hiddenlink\">";
						String link2 = "<a href=\"" + fqName
								+ HTMLReportGenerator.reportFileExt + "#"
								+ fqName + ".ctor_changed(" + type
								+ ")\" class=\"hiddenlink\">";
						String id = pkgDiff.name_ + "." + classDiff.name_
								+ ".ctor("
								+ HTMLReportGenerator.simpleName(type) + ")";
						String title = link1 + "Class <b>" + classDiff.name_
								+ "</b></a>, " + link2 + "constructor <b>"
								+ classDiff.name_ + "("
								+ HTMLReportGenerator.simpleName(type)
								+ ")</b></a>";
						memberDiff.documentationChange_ = Diff.saveDocDiffs(
								pkgDiff.name_, classDiff.name_, oldCtor.doc_,
								newCtor.doc_, id, title);
					}
					String modifiersChange = oldCtor.modifiers_
							.diff(newCtor.modifiers_);
					if (modifiersChange != null
							&& modifiersChange
									.indexOf("Change from deprecated to undeprecated") != -1) {
						System.out
								.println("JDiff: warning: change from deprecated to undeprecated for a constructor in class"
										+ newClass.name_);
					}
					memberDiff.addModifiersChange(modifiersChange);
					if (trace)
						System.out
								.println("    The single constructor was changed");
					classDiff.ctorsChanged.add(memberDiff);
					singleCtor = true;
				} else {
					if (trace)
						System.out.println("    Constructor " + oldClass.name_
								+ " was removed");
	 ...
	 * 
	 * We have to deal with the cases where: - there is only one method with a
	 * given name, but its signature changes - there is more than one method
	 * with the same name, and some of them may have signature changes The
	 * simplest way to deal with this is to make the MethodAPI comparator check
	 * the params and return type, as well as the name. This means that changing
	 * a parameter's type would cause the method to be seen as removed and
	 * added. To avoid this for the simple case, check for before recording a
	 * method as removed or added.
	 */
	public boolean compareAllMethods(ClassAPI oldClass, ClassAPI newClass,
			ClassDiff classDiff) {
		if (trace)
		...
		while (iter.hasNext()) {
			...
			// java.lang.Byte.toString(byte b) returning -16 when the compareTo
			// method
			// returned 0 on entry 13. Changed to use arrays instead, so maybe
			// it was
			// an issue with methods having another List of params used
			// indirectly by
			...
			if (idx < 0) {
				...
				if (startOld != -1 && startOld == endOld && startNew != -1
						&& startNew == endNew) {
					MethodAPI newMethod = (MethodAPI) (newClass.methods_
							.get(startNew));
	 ...
	 */
	public boolean compareMethods(MethodAPI oldMethod, MethodAPI newMethod,
			ClassDiff classDiff) {
		MemberDiff methodDiff = new MemberDiff(oldMethod.name_);

		//SK - add modifiers to methodDiff Objects
		methodDiff.setModifiersVisibility(oldMethod.modifiers_.visibility);
		...
		int inh = changedInheritance(oldMethod.inheritedFrom_,
				newMethod.inheritedFrom_);
		if (inh != 0)
			differs = true;
		if (inh == 1) {
			methodDiff
					.addModifiersChange("Method was locally defined, but is now inherited from "
							+ linkToClass(newMethod, true) + ".");
			methodDiff.inheritedFrom_ = newMethod.inheritedFrom_;
		} else if (inh == 2) {
			methodDiff.addModifiersChange("Method was inherited from "
					+ linkToClass(oldMethod, false)
					+ ", but is now defined locally.");
		} else if (inh == 3) {
			methodDiff.addModifiersChange("Method was inherited from "
					+ linkToClass(oldMethod, false)
					+ ", and is now inherited from "
					+ linkToClass(newMethod, true) + ".");
			methodDiff.inheritedFrom_ = newMethod.inheritedFrom_;
		}
		// Abstract or not
		...
		// Native or not
		if (Diff.showAllChanges && oldMethod.isNative_ != newMethod.isNative_) {
			String changeText = "";
		...
		if (docChanged(oldMethod.doc_, newMethod.doc_)) {
			...
			String link1 = "<a href=\"" + fqName
					...
					+ "\" class=\"hiddenlink\">";
			String link2 = "<a href=\"" + fqName
					+ HTMLReportGenerator.reportFileExt + "#" + fqName + "."
					+ newMethod.name_ + "_changed(" + sig
					+ ")\" class=\"hiddenlink\">";
			String id = pkgDiff.name_ + "." + classDiff.name_ + ".dmethod."
					+ newMethod.name_ + "("
					+ HTMLReportGenerator.simpleName(sig) + ")";
			String title = link1 + "Class <b>" + classDiff.name_ + "</b></a>, "
					+ link2
					+ HTMLReportGenerator.simpleName(methodDiff.newType_)
					+ " <b>" + newMethod.name_ + "("
					+ HTMLReportGenerator.simpleName(sig) + ")</b></a>";
			methodDiff.documentationChange_ = Diff.saveDocDiffs(pkgDiff.name_,
					classDiff.name_, oldMethod.doc_, newMethod.doc_, id, title);
			differs = true;
		}

		// All other modifiers
		String modifiersChange = oldMethod.modifiers_
				.diff(newMethod.modifiers_);
		if (modifiersChange != null) {
			differs = true;
			if (modifiersChange
					.indexOf("Change from deprecated to undeprecated") != -1) {
				System.out
						.println("JDiff: warning: change from deprecated to undeprecated for method "
								+ classDiff.name_ + "." + newMethod.name_);
		...
		if (differs) {
			if (trace) {
				System.out.println("    Method " + newMethod.name_
						+ " was changed: old: " + oldMethod.returnType_ + "("
						+ oldSig + "), new: " + newMethod.returnType_ + "("
						+ newSig + ")");
				if (methodDiff.modifiersChange_ != null)
					System.out.println("    Modifier change: "
							+ methodDiff.modifiersChange_);
			}
	...
	public boolean compareAllFields(ClassAPI oldClass, ClassAPI newClass,
		...
		while (iter.hasNext()) {
			...
			if (idx < 0) {
				...
				if (existsNew != -1) {
					FieldAPI newField = (FieldAPI) (newClass.fields_
							.get(existsNew));
					if (oldField.inheritedFrom_ == null
							|| newField.inheritedFrom_ == null) {
						// We also know that one of the fields is locally
						// defined.
						MemberDiff memberDiff = new MemberDiff(oldField.name_);
						memberDiff
								.setModifiersVisibility(oldField.modifiers_.visibility);
						memberDiff.oldType_ = oldField.type_;
						memberDiff.newType_ = newField.type_;
						// Changes in inheritance
						int inh = changedInheritance(oldField.inheritedFrom_,
								newField.inheritedFrom_);
						if (inh != 0)
							differs = true;
						if (inh == 1) {
							memberDiff
									.addModifiersChange("Field was locally defined, but is now inherited from "
											+ linkToClass(newField, true) + ".");
							memberDiff.inheritedFrom_ = newField.inheritedFrom_;
						} else if (inh == 2) {
							memberDiff
									.addModifiersChange("Field was inherited from "
											+ linkToClass(oldField, false)
											+ ", but is now defined locally.");
						} else if (inh == 3) {
							memberDiff
									.addModifiersChange("Field was inherited from "
						...
						if (oldField.value_ != null
							...
							String changeText = "Changed in value from "
									+ oldField.value_ + " to "
									+ newField.value_ + ".";
							memberDiff.addModifiersChange(changeText);
							differs = true;
						...
						if (docChanged(oldField.doc_, newField.doc_)) {
							...
							String link2 = "<a href=\"" + fqName
									+ HTMLReportGenerator.reportFileExt + "#"
									+ fqName + "." + newField.name_
									+ "\" class=\"hiddenlink\">";
							String id = pkgDiff.name_ + "." + classDiff.name_
									+ ".field." + newField.name_;
							String title = link1
									+ "Class <b>"
									+ classDiff.name_
									+ "</b></a>, "
									+ link2
									+ HTMLReportGenerator
											.simpleName(memberDiff.newType_)
									+ " <b>" + newField.name_ + "</b></a>";
							memberDiff.documentationChange_ = Diff
									.saveDocDiffs(pkgDiff.name_,
											classDiff.name_, oldField.doc_,
											newField.doc_, id, title);
							differs = true;
						}

						// Other differences
						String modifiersChange = oldField.modifiers_
								.diff(newField.modifiers_);
						memberDiff.addModifiersChange(modifiersChange);
						if (modifiersChange != null
								&& modifiersChange
										.indexOf("Change from deprecated to undeprecated") != -1) {
						...
						if (trace)
							System.out.println("    Field " + newField.name_
									+ " was changed");
					...
					if (trace)
						System.out.println("    Field " + oldField.name_
								+ " was removed");
	 ...
	 * 
	 * @return true if both are non-null and differ, or if one is null and the
	 *         other is not.
	 */
	 ...
	 * 
	 * @return 0 if both are null, or both are non-null and are the same. 1 if
	 *         the oldInherit was null and newInherit is non-null. 2 if the
	 *         oldInherit was non-null and newInherit is null. 3 if the
	 *         oldInherit was non-null and newInherit is non-null and they
	 *         differ.
	 */
	...
	/**
	 * Given the name of the class, generate a link to a relevant page. This was
	 * originally for inheritance changes, so the JDiff page could be a class
	 * changes page, or a section in a removed or added classes table. Since
	 * there was no easy way to tell which type the link should be, it is now
	 * just a link to the relevant Javadoc page.
	 */
	public static String linkToClass(String className, String memberName,
		...
		if (cls == null) {
			if (useNew)
				System.out
						.println("Warning: class "
								+ className
								+ " not found in the new API when creating Javadoc link");
			else
				System.out
						.println("Warning: class "
								+ className
								+ " not found in the old API when creating Javadoc link");
		...
		if (clsIdx != -1) {
			...
			String res = "<a href=\"" + prefix + pkgRef + cls.name_ + ".html#"
					+ memberName;
			if (memberType != null)
				res += "(" + memberType + ")";
